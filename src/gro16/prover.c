...
#pragma omp parallel for
    ...
    for (int j = 0; j < pk->qap_size; j += 3)
    {
        ...
        {
        case 1:
            mclBnFr_add(&AsFr[pk->LRO[j + 1]], &AsFr[pk->LRO[j + 1]], &uw[pk->LRO[j + 2]]);
            break;
        case 2:
            mclBnFr_add(&BsFr[pk->LRO[j + 1]], &BsFr[pk->LRO[j + 1]], &uw[pk->LRO[j + 2]]);
            break;
        case 3:
            mclBnFr_add(&CsFr[pk->LRO[j + 1]], &CsFr[pk->LRO[j + 1]], &uw[pk->LRO[j + 2]]);
            break;
        case 10:
        {
            mclBnFr factorFr;
...
void mul_exp(struct mulExpResult* result, mclBnFr* uwProof, proving_key* pk)
{
    ...
    mclBnG1_mulVecMT(&result->uwC1, pk->pk1, uw + nPublic + nConst, M - (nPublic + nConst),
                     num_threads);
           ...
           proving_key* pk)
{
    ...
    elapsed += (end.tv_nsec - begin.tv_nsec) / 1000000000.0;
    if (bench)
        printf("  |--- Compute h coefficients:  [%fs]\n", elapsed);
    ...
    if (bench)
        printf("  |--- G1, G2 multiexponentiations:  [%fs]\n", elapsed);

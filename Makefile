...
ifeq ($(ARCH), x86)
	...
	$(CC) $(COMMON) $(MCLINCL) $(EXTLIBMAC) -D $(CURVE) $(MULTI_SET) -D IS_MAC_OS
...
ifeq ($(shell uname), Darwin)
	$(CC) tests/test_zpie.c -o build/test_zpie build/libzpie.a $(MCLLIB) -I ./include $(MCLINCL) $(EXTLIBMAC) -D $(CURVE) $(MULTI_SET) -D IS_MAC_OS
else
...
ifneq ("$(wildcard build/libzpie.a)","")
	...
	make
ifeq ($(shell uname), Darwin)
	$(CC) tests/bench_zpie.c -o build/bench_zpie build/libzpie.a $(MCLLIB) -I ./include $(MCLINCL) $(EXTLIBMAC) -D $(CURVE) $(MULTI_SET) -D IS_MAC_OS
else
